...
test_one ()
{
	...
	if [ $output = "died" ];then
		printf "${_GREEN} Test #1 Succeeded!${_END}\n"
	else
		printf "${_RED} Test #1 Failed!${_END}\n"
...
test_two ()
{
	printf "${_CYAN} Test #2: Executing your program for 180 second, please wait...${_END}\n"
	...
	while [ $i -lt 180 ]; do
		...
		pgrep $1 > /dev/null
		if [ "$?" -ne 0 ]; then
		printf "\r${_RED} Test #2 Failed!${_END}\n"
			error_log $1 "Test #2" "Given 4 410 200 200 arguments to $1, no philosopher should die! - is dead at $i second"
			error=1
			break
		fi
		sleep 1
	...
	if [ $error -eq 0 ]; then
		pkill $1
		printf "${_GREEN} Test #2 Succeeded!${_END}\n"
	fi
}

test_three ()
{
	printf "${_CYAN} Test #3: Executing your program for 180 second, please wait...${_END}\n"
	...
	while [ $i -lt 180 ];do
		...
		pgrep $1 > /dev/null
		if [ "$?" -ne 0 ];then
			printf "\r${_RED} Test #3 Failed!${_END}\n"
			error_log $1 "Test #3" "Given 4 800 200 200 arguments to $1, no philosopher should die! - is dead at $i second"
			error=1
			break
		fi
		sleep 1
	...
	if [ $error -eq 0 ];then
		pkill $1
		printf "${_GREEN} Test #3 Succeeded!${_END}\n"
	fi
}
...
tests ()
{
	...
	printf "${_CYAN}Tests #4 in progress, please wait...${_END}\n"
